...
Реквизиты кнопки "Перевести по СБП" в Telegram/MAX-ботах можно переопределить переменными окружения:
...
Основные команды и кнопки: `/debts`, `/house 12`, `/link 12`, `/me`, `/pay 12 1500 комментарий`, "Перевести по СБП". Если дом не привязан, `/link 12` или кнопка "Привязать дом" отправляет заявку администратору. Дата платежа ставится автоматически текущим днем. Для платежа обязательно приложить фото-скрин; если дом привязан к Telegram-аккаунту, через кнопку "Отправить платеж" можно вводить только сумму.
